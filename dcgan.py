...
_config.gpu_options.per_process_gpu_memory_fraction = 0.30
...
class DCGAN():
    ...
    def build_discriminator(self):
        ...
        model.add(Conv2D(512, kernel_size=5, strides=2))
        model.add(LeakyReLU(alpha=0.2))
        model.add(Dropout(0.25))
  
        model.add(Conv2D(1024, kernel_size=(4,3), strides=1))
        model.add(LeakyReLU(alpha=0.2))
        model.add(Dropout(0.25))
        model.add(Flatten())
    ...
    def train(self, epochs, batch_size=32, sample_interval=50):
        ...
        for epoch in range(epochs):
            ...
            if epoch % 5000 == 0 :
                ...
                self.generator.save_weights('./dcgan_model/generator{}.hdf5'.format(ep))
                self.discriminator.save_weights('./dcgan_model/discriminator{}.hdf5'.format(ep))
